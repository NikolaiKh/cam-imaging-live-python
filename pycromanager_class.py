...
class MMcamera():
    ...
    def get_buffer_image(self):
        # self.instr.snap_image()
        buffer_size = self.instr.get_image_buffer_size()
        # tagged_image = self.instr.get_last_image()
        tagged_image = self.instr.get_tagged_image()
        img = np.reshape(tagged_image.pix, \
                         newshape=[tagged_image.tags['Height'], tagged_image.tags['Width']]).astype(int)
        self.param['Height'] = tagged_image.tags['Height']
...
if __name__ == "__main__":
    camera = MMcamera()
    # img = camera.get_buffer_image()
    camera.set_exposure(100)

    # print(f"Exposure time {camera.getExptime()} ms")
    ...
    print(f"All ReadoutRates options: {camera.get_allReadoutRates()}")
    print(f"All TriggerModes options: {camera.get_allTriggerModes()}")
    print(f"Current trigger mode: {camera.get_TriggerMode()}")
    camera.set_TriggerMode("Timed")
    print(f"Current trigger mode: {camera.get_TriggerMode()}")
    camera.set_TriggerMode("Strobed")
    print(f"Current trigger mode: {camera.get_TriggerMode()}")
    # readout = camera.get_allReadoutRates()
    # camera.set_ReadoutRate(readout[1])
